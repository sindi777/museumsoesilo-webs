...
import FormKoleksi from "./form";
import { FaEdit, FaTrash, FaPlusCircle, FaBoxOpen } from "react-icons/fa";
import { deleteKoleksi } from "./actions"; // Import action
...
export default function KoleksiClient({ koleksiList }) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteKoleksi(id);
      // Tidak perlu router.refresh(), revalidatePath di action sudah menanganinya
    } catch (error) {
      alert(error.message);
    }
  };
  ...
  const handleFormFinish = () => {
    setShowForm(false);
    setEditingItem(null);
    // Tidak perlu router.refresh() lagi
  };

  return (
    <div className="space-y-10">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#33231A]">
          📦 Manajemen Koleksi
        </h1>
        <button
          onClick={() => {
      ...
      {showForm && (
        <FormKoleksi initialData={editingItem} onFinish={handleFormFinish} />
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {koleksiList.map((item) => (
            ...
            >
              ...
              />
              <div className="p-4 space-y-2">
                <h2 className="text-lg font-semibold text-[#3B2C24] line-clamp-1">
                  {item.nama}
                </h2>
